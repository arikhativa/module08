...
Span::Span (const Span &src) : _max_size (src._max_size), _set (src._set) {}
...
std::ostream &
operator<< (std::ostream &o, Span const &i)
{
  o << "set size: " << i.getSet ().size () << " [";
  for (std::multiset<int>::const_iterator it = i.getSet ().begin ();
       it != i.getSet ().end (); it++)
    {
      o << *it;
      if (it != --i.getSet ().end ())
...
Span::shortestSpan (void) const
{
  ...
  int min = INT_MAX;
  for (std::multiset<int>::const_iterator it = _set.begin ();
       it != _set.end (); it++)
    {
